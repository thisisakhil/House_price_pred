...
model = joblib.load('/app/Code/model.pkl')
...
with st.sidebar.expander("House Features"):
    ...
    grliv_area = st.number_input(
        'Above Ground Living Area (sqft)', min_value=0, value=1500)
    year_built = st.number_input('Year Built', min_value=1800, value=2000)
    total_bsmt_sf = st.number_input(
        'Total Basement Area (sqft)', min_value=0, value=1000)
    full_bath = st.number_input(
        'Number of Full Bathrooms Above Ground', min_value=0, value=2)
    half_bath = st.number_input(
        'Number of Half Bathrooms Above Ground', min_value=0, value=1)
    garage_cars = st.number_input(
        'Garage Size (in Car Capacity)', min_value=0, value=2)
